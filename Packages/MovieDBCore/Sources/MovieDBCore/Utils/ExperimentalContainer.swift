...
public class ExperimentalContainer {
  ...
  static var shared = ExperimentalContainer()
  
  private var storage: [ObjectIdentifier: any InjectionBox] = [:]
  
  func resolve<T>(_ instantiator: Instantiator<T>) -> T {
...
public struct Instantiator<T> {
  ...
  let make: () -> T
  init(strategy: Strategy, closure: @escaping () -> T) {
    self.strategy = strategy
    self.make = closure
  }
  
  public static func singleton<T>(_ closure: @escaping () -> T) -> Instantiator<T> {
    .init(strategy: .singleton, closure: closure)
  }
  
  public static func shared<T>(_ closure: @escaping () -> T) -> Instantiator<T> {
    .init(strategy: .shared, closure: closure)
  }
  
  public static func onDemand<T>(_ closure: @escaping () -> T) -> Instantiator<T> {
...
@propertyWrapper public final class Inject<T> {
  ...
  let container = ExperimentalContainer.shared
  var localInstance: T?
  ...
  public var wrappedValue: T {
    get {
      ...
      localInstance = container.resolve(instantiator)
      return localInstance!
    }
  }
